document.addEventListener('DOMContentLoaded', function() {
    // Filter by urgency
    const filterUrgency = document.getElementById('filter-urgency');
    filterUrgency.addEventListener('change', function() {
        const selectedUrgency = filterUrgency.value;
        const rows = document.querySelectorAll('#requests-list tr');
        
        rows.forEach(row => {
            const urgency = row.querySelector('td:nth-child(5) span').textContent.toLowerCase();
            if (selectedUrgency === 'all' || urgency === selectedUrgency) {
                row.style.display = '';
            } else {
                row.style.display = 'none';
            }
        });
    });

    // Search by name
    const searchInput = document.getElementById('search');
    searchInput.addEventListener('input', function() {
        const searchText = searchInput.value.toLowerCase();
        const rows = document.querySelectorAll('#requests-list tr');
        
        rows.forEach(row => {
            const studentName = row.querySelector('td:nth-child(2)').textContent.toLowerCase();
            if (studentName.includes(searchText)) {
                row.style.display = '';
            } else {
                row.style.display = 'none';
            }
        });
    });

    // Handle action buttons (Approve, Reject, View Details)
    const actionButtons = document.querySelectorAll('#requests-list button');
    actionButtons.forEach(button => {
        button.addEventListener('click', function() {
            const row = button.closest('tr');
            const studentName = row.querySelector('td:nth-child(2)').textContent;
            const amount = row.querySelector('td:nth-child(4)').textContent;
            const urgency = row.querySelector('td:nth-child(5) span').textContent;

            if (button.textContent === 'Approve') {
                alert(`Request from ${studentName} of ${amount} has been approved.`);
            } else if (button.textContent === 'Reject') {
                alert(`Request from ${studentName} of ${amount} has been rejected.`);
            } else if (button.textContent === 'View Details') {
                alert(`Details:\nStudent: ${studentName}\nAmount: ${amount}\nUrgency: ${urgency}`);
            }
        });
    });
});
